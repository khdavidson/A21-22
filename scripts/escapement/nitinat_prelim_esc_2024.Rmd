...
ggplot(data=nrh.co %>%
         filter(ParentActivityType=="Adult Capture", grepl("Lake", InventoryLot, ignore.case=T), 
                grepl("Seine", `Capture Method`)) %>% 
         group_by(`Activity Start Date`, `External Mark`) %>% 
         summarize(n=sum(TotalCount)) %>% 
         group_by(`Activity Start Date`) %>% 
         mutate(mark_rate = n/sum(n))) +
  geom_bar(aes(x=`Activity Start Date`, y=mark_rate, fill=`External Mark`, colour=`External Mark`), 
...
co_markrateALL <- nrh.co %>%
  filter(ParentActivityType=="Adult Capture", grepl("Lake", InventoryLot, ignore.case=T), 
         grepl("Seine", `Capture Method`)) %>% 
  group_by(`External Mark`) %>% 
  summarize(n=sum(TotalCount)) %>% 
  mutate(mark_rate = n/sum(n)) %>%
  rename(ExternalMark=`External Mark`)

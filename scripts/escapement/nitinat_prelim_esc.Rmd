...
AUCdataTable <- readxl::read_excel(path=paste0("//dcbcpbsna01a.ENT.dfo-mpo.ca/SCD_Stad/WCVI/ESCAPEMENT/Data/",
                                               analysis_year,
                                               "/SILs/Area 21-22/AUC_DataAnalysisTable.xlsx"))
  #readxl::read_excel(path=here("data", "escapement", "prelim-inseason", "AUC_DataAnalysisTable_area21-22.xlsx"))
...
ggarrange(
  ...
  ggplot() +
    ...
    geom_bar(data=nit.sil %>% 
               ...
               summarize(total_adults = sum(`adults live`)) %>% 
               filter(date==as.Date("2023-10-05")),
             aes(x=as.Date(date), y=1), stat="identity", fill="transparent", colour="black", width=5) +
    scale_x_date(breaks="5 day", date_labels="%b %d") +
    scale_y_continuous(breaks=seq(0,1,by=0.1)) +
    labs(x="", y="Percent of total adult count") +
    theme_bw() +
    theme(axis.text.x = element_text(angle=45, hjust=1)) +
    facet_wrap(~species),
...
Figure: Number and proportion of counts in segment 5-4 (pink) compared to the other segments (blue) for all species. Black outline indicates the day where segment 5-4 was approximated from afar. 

<br>

Notes:

+ The survey following the Oct 5 incomplete survey was Oct 12, which was the flight and so had no reach-specific counts. 

+ The Chinook count on Oct 5 in segment 5-4 was much less than what was observed the survey before (Sept 29). In the survey before, segment 5-4 accounted for ~13% of the population (or in other words, 87% existed outside of segment 5-4). By Oct 26 there were very few Chinook left in the system. Reach 5-4 is mostly a holding area, so we would expect a few more Chinook holding in the deeper pool on Oct 5 than noted (fish would still be coming into spawn), but it is very close to Chinook spawning so they likely would have moved off into spawning grounds around this time or very soon after given the rains were already happening. **Therefore recommend applying 95% pop for Chinook**. 

+ The Chum count on Oct 5 in segment 5-4 is less than observed Sept 29, but not much less. In the survey before, segment 5-4 accounted for about 20% of the population (or conversely, 80% existed outside of segment 5-4). There were still fish in segment 5-4 on Oct 26, but obviously declining numbers as Chum were moving into the spawning grounds from holding pools. We would expect then that on Oct 5 there should still have been a considerable number holding, especially because they peaked the week after on the Oct 12 heli survey. **Therefore recommend applying 90% of pop to Chum**. 

+ The Coho count is also considerably less on Oct 5 compared to Sept 29 for segment 5-4. On Sept 29 segment 5-4 accounted for about 20% of the population (or in other words, 80% existed outside of segment 5-4). However, there were no coho in that segment Oct 26, which suggests that although Coho come into the river and hold for a long time before spawning, segment 5-4 may not be a preferred holding location, and they  may shoot up to hold closer to tributaries or in even deeper pools (Eagle Pool has infilled in recent years). **Therefore recommend applying 90% of pop to Coho**. 
...
For 2023 estimate we went with PL+D as even the highest AUC estimate didn't cover the peak raw live count and timing/coverage wasn't great for AUC (no spawning observed). Expanded PL+D = 1364 adults + 154 jacks (1518 total). ADDED swim-ins and angled catch due to survey timing relative to swim ins this year (see graph above - swim ins were already in the hatchery by the time the PL+D count happened): 1092 adult swim-ins and 58 jacks, 16 angled adults and 3 angled jacks. With swim-ins and angling added, final escapement estimate is  1364+1092+16 = 2472 adults + 215 jacks (2687 total). See beginning section for commentary about expansions for omission of Reach 5-4 on Oct 5 survey due to bear activity. *This is subject to change; see EscBatchHardCopy report for specific estimate abundance.*
...
For 2023 used AUC with SL=11.5 days (mid value) to give expanded estimate of 27,530. Swim-ins need to be SUBTRACTED from this to account for fish that were likely double-counted: the peak live count occurred on the full system heli survey and therefore likely counted chum that later swam into the hatchery (see swim-in graph above). Subtracting the swim-ins for accounting (6186) leaves 21,344 chum. See beginning section for commentary about expansions for omission of Reach 5-4 on Oct 5 survey due to bear activity. *This is subject to change; see EscBatchHardCopy report for specific estimate abundance.*
...
```{r}
ggplot() +
  geom_line(data=escEstAUC%>%filter(ShortSpecies=="CN", AREA!="20", grepl("Nitinat", SystemName)), 
            aes(x=as.Date(DateRange), y=ExpAdultFishDays)) +
  geom_point(data=escEstAUC%>%filter(ShortSpecies=="CN", AREA!="20", grepl("Nitinat", SystemName)), 
             aes(x=as.Date(DateRange), y=ExpAdultFishDays, fill=method, colour=method), shape=21, size=4) +
  scale_fill_manual(breaks=c("heli", "snorkel"), values=c("orange", "blue")) +
  scale_colour_manual(breaks=c("heli", "snorkel"), values=c("orange", "blue")) +
  labs(x="", y="Fish-days", fill="Survey method", colour="Survey method") +

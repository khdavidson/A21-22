...
nitinat.SIL <- readxl::read_excel(path=list.files(path=paste0(here::here("data", "escapement", "prelim-inseason", escapement_year)),
  ...
  mutate(SBJ_ID = case_match(SBJ_ID,
         ...
         survey_type = case_when(STREAM_MTP_ID==6 & SEGMENT_NAME %in% c("35-34", "34-29", "29-23", "23-17", 
                                 ...
                                 TRUE ~ "FLAG")) %>%
  filter(!grepl("san juan|gordon|harris|lens|renfrew", NAME, ignore.case=T))
...
ggplot() +
  ...
  geom_bar(data=nitinat.SIL %>% 
           ...
           aes(x=date, y=total_adults, group=SBJ_ID, fill=SBJ_ID), stat="identity", alpha=0.8, width=7) +
...
ggplot() +
  ...
  geom_bar(data=nitinat.SIL %>% 
           ...
           stat="identity", alpha=0.8, width=5, size=1) +
...
ggplot() +
  geom_bar(data=nitinat.SIL %>% 
           ...
           stat="identity", position="stack", alpha=0.7, linewidth=1.1, width=5) +
...
ggplot()+
  ...
  geomtextpath:: geom_textline(data=nitinat.water %>%
                              ...
                              colour="gray40", linewidth=1, show.legend=F, hjust=0.45, text_smoothing=30, alpha=0.6) +
  geomtextpath:: geom_labelline(data=nitinat.water %>% 
                              ...
                              colour="dodger blue", linewidth=1.2, hjust=0.031, show.legend=F) +
...
Graphs focusing on the lead up to and including peak salmon periods. Notable events included two lake turnover events this year, one on Sept 17 and the other Oct 14. On Sept 17 the hatchery lost all Chinook brood held in lake pens (~2800). The second turnover may have just been partial but rotten egg smell was noted especially at top end of lake.

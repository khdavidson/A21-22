...
ggplot()+
  geom_ribbon(data=SJ.water %>% 
              ...
              aes(x=as.Date(DOY, origin="2021-12-31"), ymin=min_lvl, ymax=max_lvl), fill="gray80", alpha = 0.7) +
  geomtextpath:: geom_textline(data=SJ.water %>% 
                               ...
                               aes(x=as.Date(DOY, origin="2021-12-31"), y=mean_lvl_hist, label=as.factor(label)), 
                               colour="gray60", linewidth=1, show.legend=F, hjust=0.2, text_smoothing=30, alpha=0.8) +
  geom_ribbon(data=nitinat.water %>% 
  ...
  geomtextpath:: geom_textline(data=nitinat.water %>% 
                               ...
                               aes(x=as.Date(DOY, origin="2021-12-31"), y=mean_lvl, label=as.factor(label)), 
                               colour="dodger blue", linewidth=1, show.legend=F, hjust=0.17, alpha=0.8) +
  labs(y="Water level (m)") +
  scale_colour_manual(values=c("dodger blue", "gray70")) +
  scale_x_date(date_labels = "%b %d") +
  scale_y_continuous(breaks=seq(0,10,by=0.5)) +
...
```{r fig.cap='Raw live (bar) and carcass (line) counts for Nitinat River. Not all surveys are full system: early surveys are lower river only. Dead counts will populate once die-off begins.'}
ggplot() +
  # ---- DEAD -----
...
ggplot() +
  ...
  geom_line(data=nitinat.water %>% 
              ...
              group_by(date2) %>% 
              summarize(mean_water = mean(Water.Level.m), min=min(Water.Level.m), max=max(Water.Level.m)),
            aes(x=date2, y=mean_water*10), colour="dodger blue", size=1) +
  # ---- COUNTS ----
  geom_bar(data=nitinat.SIL %>% 
             filter(system=="Nitinat River", species!="No fish") %>% 
             group_by(date, species) %>% 
  ...
  scale_x_date(date_breaks="7 day", date_labels = "%b %d") +
  scale_y_continuous(sec.axis = sec_axis(~ . /10, name = "water level (m)")) +
  labs(x="", y="Live count") +
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, hjust=1),
        legend.position = "none",

...
esc <- readxl::read_excel(path = list.files(path = here::here("outputs"),
                          ...
                          sheet=1) %>%
  janitor::clean_names() %>%
  filter(waterbody_name=="HOBITON CREEK", species=="Chum") %>%
...
```{r hob-esc, fig.cap='Recent Chum escapement to Hobiton Creek. Years without bars mean no data are in NuSEDS, but there may be data available from DFN/NTC echosounder counts not yet processed/provided. Data from NuSEDS, Nov 2024.'}

ggplot() +
  geom_bar(data = esc %>%
             filter(est_type %in% c("Natural Adult Spawners")),
           aes(x=year, y=estimate), stat="identity", fill="#26407d", colour="#26407d", alpha=0.8, linewidth=1) +
  geom_bar(aes(x=2025, y=700), stat="identity", colour="red", fill="white", linetype="dashed", linewidth = 1) +
  scale_x_continuous(breaks=seq(1950, 3000, by=1), limits=c(2000, 2026)) +
  labs(y="Natural spawners") +
  theme_bw() +

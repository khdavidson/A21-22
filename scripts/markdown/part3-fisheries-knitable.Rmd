...
ggplot(data = WCVIcrestBDWR.catEst.Nit %>% 
  ...
  geom_point(aes(y=AREA, x=factor(MONTH, levels=month.name), fill=NitOnly_forPlot, colour=NitOnly_forPlot, 
                 text=paste("Estimated # kept Nitinat CN=", round(estKeptByID,0), "\nBased on # confirmed biosamples, n=", n_lvl2)), 
             shape=22, size=6, alpha=0.8) +
  scale_fill_gradient(low="#2900aa", high="orange", na.value=alpha("gray80", 0.3))+
  scale_colour_gradient(low="#2900aa", high="orange", na.value=alpha("gray80", 0.3))+
  # --- colour options below are nice but don't work with ggplotly: 
